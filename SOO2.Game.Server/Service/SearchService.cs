    ...
    public class SearchService : ISearchService
    {
        ...
        public void OnChestDisturbed(NWPlaceable oChest)
        {
            ...
            else if (disturbType == INVENTORY_DISTURB_TYPE_REMOVED)
            {
                ...
                oPC.AssignCommand(() =>
                {
                    _.ActionPlayAnimation(ANIMATION_LOOPING_GET_LOW, 1.0f, searchDelay);
                });

                _.ApplyEffectToObject(DURATION_TYPE_TEMPORARY, _.EffectCutsceneImmobilize(), oPC.Object, searchDelay);
        ...
        public void OnChestOpen(NWPlaceable oChest)
        {
            ...
            if (_.GetActionMode(oPC.Object, ACTION_MODE_STEALTH) == TRUE)
                _.SetActionMode(oPC.Object, ACTION_MODE_STEALTH, FALSE);
            
            string resref = oChest.Resref;
            int chestID = oChest.GetLocalInt(SearchSiteIDVariableName);
            int skillRank = _.GetSkillRank(SKILL_SEARCH, oPC.Object);
        ...
        public void OnChestUsed(NWPlaceable oChest)
        {
            ...
            NWPlaceable oCopy = NWPlaceable.Wrap(_.CreateObject(OBJECT_TYPE_PLACEABLE, SearchSiteCopyResref, oChest.Location));
        ...
        private void RunSearchCycle(NWPlayer oPC, NWPlaceable oChest, int iDC)
        {
            ...
            int skill = _.GetSkillRank(SKILL_SEARCH, oPC.Object);
